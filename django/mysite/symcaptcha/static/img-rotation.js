    ...
    var theta0 = Math.floor(Math.random()*360);
    theta0 = 90*Math.floor(theta0/90);
    var theta1 = Math.floor(Math.random()*360);
    theta1 = 90*Math.floor(theta1/90);
    var h_img = 230, w_img = 300;
    var x0 = 0, y0 = 0, x1 = 0, y1 = 0, w = w_img, h = h_img;
    ...
    function point_it(cvs_id, event){
        ...
        var px=pos_x,py=pos_y;//the acutal ploted points
        if(cvs_id=="mycanvas")
        switch(theta0){
              case 90:
              px = pos_y;
        ...
        ctx.stroke();
        draw_line(cvs_id,ctx,pos_x,pos_y,px,py);
    }

    var lrunning = 1,rrunning = 1;
    var start_x0 = 0, start_y0 = 0, start_px0 = 0, start_py0 = 0;
    var start_x1 = 0, start_y1 = 0, start_px1 = 0, start_py1 = 0;

    var start_x0_rot = 0, start_y0_rot = 0, end_x0_rot = 0, end_y0_rot = 0;
    var start_x1_rot = 0, start_y1_rot = 0, end_x1_rot = 0, end_y1_rot = 0;
    
    function draw_line(cvs_id,ctx,x,y,px,py){
        x = Math.round(x);
        y = Math.round(y);
        px = Math.round(px);
        py = Math.round(py);
        ...
        switch(lrunning){
            ...
            case 2:
              ctx.beginPath();
              ctx.moveTo(start_px0,start_py0);
              ctx.lineTo(px,py);
              ctx.stroke();
              lrunning = 1;
              document.getElementById("coords").value = "("+start_x0.toString()+","+start_y0.toString()+"); ("+x.toString()+","+y.toString()+")";
              document.getElementById("coords_rot").value = "("+(start_px0-x0).toString()+","+(start_py0-y0).toString()+"); ("+(px-x0).toString()+","+(py-y0).toString()+")";
              break;
        }
        else
         switch(rrunning){
            ...
            case 2:
              ctx.beginPath();
              ctx.moveTo(start_px1,start_py1);
              ctx.lineTo(px,py);
              ctx.stroke();
              rrunning = 1;
              document.getElementById("test_coords").value = "("+start_x1.toString()+","+start_y1.toString()+"); ("+x.toString()+","+y.toString()+")";
              document.getElementById("test_coords_rot").value = "("+(start_px1-x1).toString()+","+(start_py1-y1).toString()+"); ("+(px-x1).toString()+","+(py-y1).toString()+")";
              break;
        }
    }
    ...
    function showOldImg(){
        ...
        img.onload = function(){
          ...
          ctx.rotate(theta0*Math.PI/180);
          ctx.drawImage(img,x0,y0,w_img,h_img);
      }
      img.src = current_img;
    ...
    function showImgFromDB(){
       ...
       testimg.onload = function(){
          ...
          ctx.drawImage(testimg,x1,y1,w_img,h_img);
       }
    ...
    function reload(){
        window.location.href = "http://sundance.ist.psu.edu:8080/symcaptcha/rot/";
        }
